...
public class RestaurantChecker {
	...
	public static CheckResult checkRestaurantVOForSave(RestaurantVO restaurantVO) {
		...
		if (StringUtils.isBlank(restaurantVO.getLogoUrl())) {
			log.error(temp, JSON.toJSONString(restaurantVO));
			return CheckResult.error("列表页展示图不能为空");
		}
		if (StringUtils.isBlank(restaurantVO.getCoverPics())) {
			log.error(temp, JSON.toJSONString(restaurantVO));
			return CheckResult.error("详情页展示图不能为空");
		}
		return CheckResult.success();
	}

	public static CheckResult checkRestaurantVOForUpdate(RestaurantVO restaurantVO) {
		String temp = "餐厅信息验证失败: {}";
