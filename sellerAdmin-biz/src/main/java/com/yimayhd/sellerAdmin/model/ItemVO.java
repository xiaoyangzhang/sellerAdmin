...
public class ItemVO extends ItemDO {
	...
	private String				itemSkuVOStr;										// sku列表
																					// jsonString
	 ...
	 * @throws Exception
	 */
	public static CommonItemPublishDTO setItemSkuDOListCommonItemPublishDTO(CommonItemPublishDTO commonItemPublishDTO,
		...
		if (CollectionUtils.isNotEmpty(categoryVO.getSellCategoryPropertyVOs())) {
			...
			for (int i = 0; i < itemSkuVOListTran.size(); i++) {
				...
				for (int j = 0; j < itemSkuPVPairListTran.size(); j++) {
					// pId + vId + vTxt决定唯一
					String str = String.valueOf(itemSkuPVPairListTran.get(j).getPId())
							+ String.valueOf(itemSkuPVPairListTran.get(j).getVId())
							+ null == itemSkuPVPairListTran.get(j).getVTxt() ? ""
									: itemSkuPVPairListTran.get(j).getVTxt();
			...
			for (int i = 0; i < itemSkuVOList.size(); i++) {
				for (int j = 0; j < itemSkuVOListAll.size(); j++) {
					if (isEqual(itemSkuVOList.get(i).getItemSkuPVPairList(),
							itemSkuVOListAll.get(j).getItemSkuPVPairList())) {
				...
				for (ItemSkuPVPair itemSkuPVPair : itemSkuVOList.get(i).getItemSkuPVPairList()) {
					for (CategoryPropertyValueVO categoryPropertyValueVO : categoryVO.getSellCategoryPropertyVOs()) {
						...
						if (itemSkuPVPair.getPId() == categoryPropertyValueVO.getCategoryPropertyVO().getId()) {
							CategoryPropertyVO categoryPropertyVO = categoryPropertyValueVO.getCategoryPropertyVO();
							if (categoryPropertyValueVO != null
									&& CollectionUtils.isNotEmpty(categoryPropertyVO.getCategoryValueVOs())) {
								for (CategoryValueVO categoryValueVO : categoryPropertyVO.getCategoryValueVOs()) {
									// vId和vTxt相同时
									if (itemSkuPVPair.getVId() == categoryValueVO.getId()
											&& itemSkuPVPair.getVTxt().equals(categoryValueVO.getText())) {
	...
	public static boolean isEqual(List<ItemSkuPVPair> list1, List<ItemSkuPVPair> list2) {
		if (CollectionUtils.isNotEmpty(list1) && CollectionUtils.isNotEmpty(list2) && list1.size() == list2.size()) {
			for (int i = 0; i < list1.size(); i++) {
				if (list1.get(i).getPId() != list2.get(i).getPId()
						|| !list1.get(i).getPTxt().equals(list2.get(i).getPTxt())
						|| list1.get(i).getVId() != list2.get(i).getVId()
						|| !list1.get(i).getVTxt().equals(list2.get(i).getVTxt())) {
					return false;
				}
			}
			return true;
