...
public class CommodityServiceImpl implements CommodityService {
	...
	public ItemResultVO getCommodityById(long sellerId, long id) throws Exception {
		...
		// h5
		PicTextResult picTextResult = pictureTextRepo.getPictureText(itemResultVO.getItemVO().getId(),
				PictureText.ITEM);
	...
	public ItemDO addCommHotel(ItemVO itemVO) throws Exception {
		...
		if (null == result) {
			log.error("ItemPublishService.publish result is null and parame: " + JSON.toJSONString(hotelPublishDTO)
					+ "and itemVO:" + JSON.toJSONString(itemVO));
			throw new BaseException("返回结果错误,酒店商品新增失败 ");
		} else if (!result.isSuccess()) {
			log.error("ItemPublishService.publish error:" + JSON.toJSONString(result) + "and parame: "
					+ JSON.toJSONString(hotelPublishDTO) + "and itemVO:" + JSON.toJSONString(itemVO));
	...
	public void addCommonItem(ItemVO itemVO) throws Exception {
		...
		ComentEditDTO comentEditDTO = PictureTextConverter.toComentEditDTO(itemPubResult.getItemId(), PictureText.ITEM,
				JSON.parseObject(itemVO.getPictureTextVOStr(), PictureTextVO.class));
	...
	public void modifyCommonItem(ItemVO itemVO) throws Exception {
		...
		if (null != itemDB) {
			...
			try {
				...
				ComentEditDTO comentEditDTO = PictureTextConverter.toComentEditDTO(itemVO.getId(), PictureText.ITEM,
						pictureTextVO);
				pictureTextRepo.editPictureText(comentEditDTO);
			} catch (Exception e) {
