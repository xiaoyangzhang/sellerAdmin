...
jQuery.validator.addMethod("faxCheck", function(value, element) {
	return this.optional(element) ||/^[1-9][0-9]{5}$/.test(value);
}, "请输入正确的邮政编码!");

jQuery.validator.addMethod("isWebSite", function(value, element) {
	return this.optional(element) ||/^([hH][tT]{2}[pP]:\/\/|[hH][tT]{2}[pP][sS]:\/\/)(([A-Za-z0-9-~]+)\.)+([A-Za-z0-9-~\/])+$/.test(value);
}, "请输入正确的网址!");
...
jQuery.validator.addMethod("isMobile", function(value, element) {
	...
	var mobile = /^(13\d|15\d|18\d|17\d|14\d)\d{5}(\d{3}|\*{3})$/;
...
jQuery.validator.addMethod("isLicense", function(value,element) {
	return this.optional(element) || /^[0-9][0-9]{12,14}$/.test(value);
}, "请正确输入号码(13到15位纯数字)!");
//纯数字
...
jQuery.validator.addMethod("require_from_group", function(value, element, options){
    ...
    return validOrNot;
}, $.validator.format("手机和固定电话至少填写一项！"));
